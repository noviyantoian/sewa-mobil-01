...
function rateOf(car: UiCar, mode: Mode): number {
  return mode === "withDriver" ? car.rateWithDriver : car.rateSelfDrive;
}

/** Client-side filter over the DB-sourced car list (mirrors the old filterCars). */
function filterList(list: UiCar[], filters: FilterState, mode: Mode): UiCar[] {
  return list.filter((c) => {
    if (filters.category !== "all" && c.category !== filters.category) return false;
    if (filters.transmission !== "all" && c.transmission !== filters.transmission) return false;
    if (filters.minSeats && c.capacity < filters.minSeats) return false;
    const rate = rateOf(c, mode);
    if (filters.minPrice && rate < filters.minPrice) return false;
    if (filters.maxPrice && rate > filters.maxPrice) return false;
    return true;
  });
}

export function SearchClient({ cars }: { cars: UiCar[] }) {
  const t = useT();
  ...
  const results = useMemo(() => {
    const list = filterList(cars, filters, mode);
    const sorted = [...list];
