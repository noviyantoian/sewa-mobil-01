...
import { bookings as seedBookings, type Booking } from "@/lib/mock/bookings";
import { cars, getCarBySlug } from "@/lib/mock/cars";

const TODAY = "2026-06-14";
const REVENUE_STATUSES: Booking["status"][] = ["confirmed", "active", "completed"];

export default function AdminOverviewPage() {
  const t = useT();
  const [queue, setQueue] = useState<Booking[]>(() =>
    seedBookings.filter((b) => b.status === "pending")
  );

  const kpi = useMemo(() => {
    const todayCount = seedBookings.filter(
      (b) => b.pickupAt.startsWith(TODAY) || b.status === "active"
    ).length;
    const available = cars.filter((c) => c.available).length;
    const revenue = seedBookings
  ...
  const recent = useMemo(
    () => [...seedBookings].sort((a, b) => b.createdAt.localeCompare(a.createdAt)).slice(0, 5),
    []
  );
  ...
  const columns: Column<Booking>[] = [
    ...
    { key: "car", header: t("admin.car"), render: (b) => <CarCell car={getCarBySlug(b.carSlug)} /> },
    {
      key: "dates",
