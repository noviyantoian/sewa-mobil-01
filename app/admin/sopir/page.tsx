...
import { bookings, type Booking } from "@/lib/mock/bookings";
import { getCarBySlug } from "@/lib/mock/cars";

type Tone = "neutral" | "success" | "accent" | "outline";
const statusTone: Record<Driver["status"], Tone> = {
  idle: "success",
  assigned: "accent",
  off: "neutral",
};
const statusDot: Record<Driver["status"], string> = {
...
export default function AdminDriversPage() {
  ...
  const confirm = (booking: Booking, driver: Driver) => {
            ...
            return (
              <div key={b.id} className="card shadow-sm flex flex-col gap-4 p-5">
                <div className="flex items-center justify-between gap-3">
                  <CarCell car={getCarBySlug(b.carSlug)} />
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="tnum text-[12px] text-[var(--color-mute)]">{driver.id}</div>
          </div>
        </div>
