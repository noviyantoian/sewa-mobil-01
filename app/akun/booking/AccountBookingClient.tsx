...
export interface BookingView {
  ...
  to: string;
  total: number;
  car: { slug: string; name: string; brand: string; exterior: string } | null;
  pickupLabel: string;
  returnLabel: string;
}
        ...
        <div className="flex flex-col gap-4 p-5 md:p-6">
          <div className="flex items-start justify-between gap-4">
            <div className="min-w-0">
              ...
              <p className="tnum mt-1 text-[13px] text-[var(--color-mute)]">{booking.code}</p>
            </div>
          ...
          <dl className="grid grid-cols-1 gap-x-6 gap-y-2.5 text-[14px] sm:grid-cols-2">
            <div className="flex items-center gap-2 text-[var(--color-body)]">
              ...
              <span className="tnum">{formatDateRange(booking.from, booking.to, locale)}</span>
            </div>
            <div className="flex items-center gap-2 text-[var(--color-body)]">
...
function Section({ title, items }: { title: string; items: BookingView[] }) {
...
export function AccountBookingClient({ bookings }: { bookings: BookingView[] }) {
  const t = useT();
  ...
  const { upcoming, past } = useMemo(() => {
    return {
      ...
      past: bookings.filter((b) => !UPCOMING.includes(b.status)),
    };
  }, [bookings]);
